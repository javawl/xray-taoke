...
public class OrderInfo extends JfModel<OrderInfo> {
    ...
    public int updateSuccNew(String appid, String userid, String tradeid, double jiemoney, double jierate, int verno, int proxystate) {
        String sql = "update `tk_orderinfo` set `appid`=?,`userid`=?,`jiemoney`=?,`jierate`=?,`verno12`=?,`state`=12,`proxystate` = ?  where `tradeid`=? and `verno12`<=0";
