...
public class WeixinApi extends AbstractService implements ApiProvider {
	...
	public ApiResult createMenu(String appid, String content) {
		...
		for (int i = 0; i < src.size(); i++) {
			...
			for (int j = 0; j < arr.size(); j++) {
				subArr.add(processMenu(arr.getJSONObject(j), false));
			}
	...
	@Before(ApiInterceptor.class)
	public ApiResult sendImg(String appid, String openid, String meadiaid) {
		ApiResult res = CustomServiceApi.sendImage(openid, meadiaid);
		return res;
	}
	...
	@Before(ApiInterceptor.class)
	public String CreateQrcode(String appid, String sceneId) {
		ApiResult apiResult = QrcodeApi.createPermanent(sceneId);
		JSONObject jsonObjec = JSONObject.parseObject(apiResult.getJson());
		String ticket = jsonObjec.getString("ticket");
		String outMessage = QrcodeApi.getShowQrcodeUrl(ticket);
		return outMessage;
	}

	@Before(ApiInterceptor.class)
	public ApiResult uploadImg(String appid, String path) {
		File file = new File(path);
		ApiResult apiResult = MediaApi.addMaterial(file, MediaType.IMAGE);
		return apiResult;
	}
