...
public class TkMaterial extends JfModel<TkMaterial> {
    ...
    public void updateJingXuan(String appid, String content) {
        String sql = "update `tk_mpmaterial` set `content`=? where `appid`=? and `mediaid`=?";
        Db.use(Constant.db_dataSource).update(sql, content, appid, "wx_1e61011294bafd3c");
    }
