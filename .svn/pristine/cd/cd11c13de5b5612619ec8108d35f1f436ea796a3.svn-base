...
public class OrderInfo extends JfModel<OrderInfo> {
    ...
    public List<OrderInfo> queryProxy() {
        String sql = "select * from `tk_orderinfo` where `userid`!='' and `proxystate`>=1 and `proxystate` != `tkstatus`";
        return dao.find(sql);
    }

    public List<OrderInfo> queryProxyList() {
        String sql = "select * from `tk_orderinfo` where `userid`!='' and `proxystate` !=`proxytkstatus`";
        return dao.find(sql);
    }
    ...
    public int updateSucc(String appid, String userid, String tradeid, double jiemoney, double jierate, int verno, int proxystate) {
        String sql = "update `tk_orderinfo` set `appid`=?,`userid`=?,`jiemoney`=?,`jierate`=?,`verno12`=?,`state`=12,`autobind`=1,`proxystate` = ? where `tradeid`=? and `verno12`<=0";
        return Db.update(sql, appid, userid, jiemoney, jierate, verno, proxystate, tradeid);
    }
