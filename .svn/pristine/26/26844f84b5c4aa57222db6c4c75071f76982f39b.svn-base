...
public class TkStatIncomeApp extends AbstractApp {
        ...
        for (TkStatIncomeItem item : list) {
            ...
            if (data == null) {
                ...
                if (item.getInt("state") != 1) {
                    ...
                    TkStatIncomeDay tmp = TkStatIncomeDay.dao.queryByDay(DateUtil.formatDate(cal.getTime(), "yyyy-MM-dd"), item.getStr("itemid"));
                    if (tmp == null)
                        continue;
                    data.set("conssum", tmp.getDouble("conssum"));
                } else {
                    data.set("conssum", 0);
                }
                data.save();
    ...
    private void income2day(TkStatIncomeDay data, TkStatIncome income) {
        ...
        if (data.getDouble("conssum") > 0)
            data.set("incomerate", income.getDouble("trademoney") / data.getDouble("conssum"));
    }
    ...
    public void runOrder(String day) {
        ...
        for (OrderInfo order : list) {
            ...
            case 13:
                break;
            default:
                data.sets.add(order.getStr("parentid"));
