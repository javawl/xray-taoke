...
public class LinkItemController extends AbstractController {
	...
	public void goTobjAct() {
		...
		String appid = conf.getStr("appid");
		TbItemVo data2 = TbItemService.instance.getItem(lid, itemid);
		int sid = conf.getInt("sid");
		String domain = MpInfoService.instance.getUrl(appid);
		String url = data2.getItempic();
		if (url.contains("lanlanlife")) {
			int i = url.indexOf("@");
			if (i != -1) {
				url = url.substring(0, i);
			...
			url = DownloadUtil.getFilePathByLanlan(url);
			data2.setItempic("http://taoke.ldpro.cn/taoke_admin" + url);
		}
		setAttr("data", data2);

		setAttr("url", domain + "/titem?sid=" + sid + "&pkey=" + pkey + "&tid=" + data2.getItemid());
	...
	public void goListByProxy() {
		...
		long pkey = getParaToLong("pkey");

		UoUser uoUser = UoUser.dao.queryByPkey(pkey);
		double rate = uoUser.getDouble("rate");
		setAttr("lid", getPara("lwid"));
		List<TbItemVo> list = TbItemService.instance.getItemList(getPara("lwid"), pageno);
		for (TbItemVo tbItemVo : list) {
			tbItemVo.setTkrate(tbItemVo.getTkrate() * rate);
		}
		setAttr("dataList", list);
		if (pageno > 10) {
			renderNull();
