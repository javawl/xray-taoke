...
public class JdOrderInfo extends JfModel<JdOrderInfo> {
    ...
    public List<JdOrderInfo> queryByParentTradeid(String tradeid) {
        String sql = "select * from `tk_orderinfo_jd` where `parentid`=? and `tradetype` = 2 ";
    ...
    public int updateSucc(String appid, String userid, String tradeid, double jiemoney, double jierate, int verno) {
        String sql = "update `tk_orderinfo_jd` set `appid`=?,`userid`=?,`jiemoney`=?,`jierate`=?,`verno12`=?,`state`=16,`autobind`=1 where `tradeid`=? and `verno12`<=0";
