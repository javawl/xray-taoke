...
public class MpInfoService {
    ...
    private static final String proxy = config.get("redis.prefix") + ".proxy.";
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Constant.toMediaid("教程"));
        System.out.println(Constant.toMediaid("推广"));
    ...
    public String getPkeyPid(String pkey) {
        String key = prefix + "pkey";
        return cache.hget(key, pkey);
    }
    ...
    public String getMaterialContent(String appid, String mediaid) {
        String key = material + appid;
        return cache.hget(key, mediaid);
    }
    ...
    public String getNoTaokeContent(String appid) {
        StringBuffer sb = new StringBuffer();
        sb.append("[爱心]该宝贝未开通返利补贴！\n");
        sb.append("[大哭]80%的商品都有返利，而它就是那剩下的20%！\n");
        sb.append("\n");
        sb.append("下单后如果没有自动跟踪,请把订单编号发给我自动统计返利哦~");
        return sb.toString();
        // String key = material + appid;
        // return cache.hget(key, "wx_a3d24f595a4cd1b3");
    }

    public String getTitleContent(String appid) {
        StringBuffer sb = new StringBuffer();
        sb.append("[爱心]亲亲，发链接给公众号，可以直接查券\n");
        sb.append("\n");
        sb.append("[拥抱]找同款，找优惠券，私聊客服哦~\n");
        sb.append("\n");
        sb.append("下单后如果没有自动跟踪,请把订单编号发给我自动统计返利哦~");
        return sb.toString();
    }
    ...
    public String getTradeSucc(String appid, UoUser user, JdOrderInfo data) {
        StringBuffer sb = new StringBuffer();
        sb.append("━┉┉┉ 绑 定 成 功 ┉┉┉━\n");
        sb.append("商品名称：").append(data.getStr("itemtitle")).append("\n");
        sb.append("订单编号：").append(data.getStr("tradeid")).append("\n");
    ...
    public String getCashNoMoney(String appid) {
        StringBuffer sb = new StringBuffer();
        sb.append("━┉┉┉ 申 请 失 败  ┉┉┉━\n");
        sb.append("亲亲，确认收货后，系统需要点时间同步哦(约1分钟)~\n");
        sb.append("\n");
        sb.append("[委屈]由于公众号平台限制，必须大于0.3园才能自动打钱哦~\n");
        sb.append("-------------------------------\n");
        sb.append("[拥抱]回复\"查询\"，查看账户信息\n");
        sb.append("\n");
        sb.append(getShopContent(appid));
    ...
    public String getCashSucc(String appid, UoUser user) {
        ...
        if (user.getInt("proxystate") == Constant.proxystate_inuse) {
            sb.append("\n");
            sb.append("成功推广人数：").append(user.getInt("proxypeople")).append("\n");
            sb.append("代理可提现金额：").append(Constant.formatPrice(user.getDouble("qbproxycash"))).append("\n");
            sb.append("代理未收货金额：").append(Constant.formatPrice(user.getDouble("qbproxyconfirm"))).append("\n");
        }
        sb.append("-------------------------------\n");
        sb.append("[拥抱]回复\"查询\"，查看账户信息\n");
        sb.append("\n");
    ...
    public String getUserInfo(String appid, UoUser user) {
        ...
        sb.append("\n");
        sb.append("\ue231\ue231<a href=\"weixin://bizmsgmenu?msgmenucontent=提现&msgmenuid=1\">申请提现</a>\ue230\ue230");
        return sb.toString();
    }

    public String getProxySucc(String appid) {
        StringBuffer sb = new StringBuffer();
        sb.append("申请成功![玫瑰]请等待工作人员进行审核~");
        return sb.toString();
    }

    public String getProxypeople(String appid) {
        StringBuffer sb = new StringBuffer();
        sb.append("成功邀请新成员![玫瑰]\n");
        sb.append("-------------------------------\n");
        sb.append("回复【查询】查看详情~");
        return sb.toString();
    }
    ...
    public void incrTcode(String sid) {
        String key = active + sid + ".tcode." + DateUtil.getToday();
        cache.incr(key);
    }
    ...
    public void addProxycode(String appid, String openid, String proxycode) {
        String key = proxy + appid;
        cache.hset(key, openid, proxycode);
    }

    public String getProxycode(String appid, String openid) {
        String key = proxy + appid;
        return cache.hget(key, openid);
    }

    public void delProxycode(String appid, String openid) {
        String key = proxy + appid;
        cache.hdel(key, openid);
    }
}
